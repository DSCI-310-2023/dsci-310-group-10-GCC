...
fire_train <- read_csv(here("results/fire_train.csv"))
accuracies <- read_csv(here("results/accuracies.csv"))
prediction_table <- read_csv(here("results/prediction_table.csv"))

fire_train_plot <- fire_train %>%
  ggpairs(aes(color = Classes))

scatter_plot <- plot_scatter_graph(data = fire_train, plot_width = 8.9,
...
line_plot <- plot_line_graph(data = accuracies, plot_width = 10,
...
wkflw_plot <-
  ...
  scale_color_manual(labels = c("Forest fire present", 
                                "Forest fire not present"), values = cb_palette)

# Saving all the graphs
ggsave(here("results/classification_regions.png"), plot = wkflw_plot,
       device = "png")
ggsave(here("results/correlation_graph.png"), plot = fire_train_plot, 
